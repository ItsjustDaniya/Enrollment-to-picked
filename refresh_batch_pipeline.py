...
def fetch_picked_rows(target_batches, gem_map):
    ...
    for i in range(n):
        ...
        out.append((uid, batch, pdate, gem_status))
    return out
...
def build_workbook(enroll_rows, picked_rows, output_path):
    ...
    for j, h in enumerate(["UserID", "Batch", "Picked Date", "GEM Status"], start=1):
        c = ws2.cell(row=1, column=j, value=h)
        c.font, c.fill = header_font, header_fill
    for i, (uid, batch, pdate_str, gem_status) in enumerate(picked_rows, start=2):
    ...
    ws2.freeze_panes = "A2"
    ws2.cell(row=1, column=6,
              value=f"Source: FlyWheel Google Sheet via Sheets API. GEM Status via Metabase label "
                    f"mapping. Refreshed: {dt.date.today().isoformat()}.").font = note_font
